...
class Config():
    training_dir = "/home/mayank_sati/Desktop/git/2/AI/QATM/data/image_save"
...
siamese_dataset = SiameseNetworkDataset(imageFolderDataset=folder_dataset, transform=transforms.Compose([transforms.Resize((Config.img_res,Config.img_res)), transforms.ToTensor()]),should_invert=False)
...
siamese_dataset = SiameseNetworkDataset(imageFolderDataset=folder_dataset_test, transform=transforms.Compose([transforms.Resize((Config.img_res,Config.img_res)), transforms.ToTensor()]),should_invert=False)
...
for loop in range(20):
    ...
    input_batch=x0.repeat(ref_batch,1,1,1)
    # input_batch=x0
    ############################
    t1=time.time()
